...
//  1. 多线程同步地初始化编译所有文件
//      - CPU 密集型
//      - 由于ParameterizedType有所改变, 所以需要重构声明的类型信息
//          复杂的, 保留source的操作, 或许不合适
//      - 需要重构identifier pool的信息
//  2. 同步加载所有的类信息, alias信息, 和import, declare_pool信息
//      - IO 密集型
//      - 1 全部完成之后
//      - 检查Import信息的正确性, 并构建出IdentifierManager, 无Local,
//          能吗? import一个成员咋办呢?
//      - 构建文件之间的引用索引文件
//  3. 异步构建所有的类的继承树/森林-RAW_TYPE
//  4. 解析类的GenericType, 然后构建完整的类的继承树, 包含父类的ParameterizedType
//      TODO 由于复杂的嵌套关系, 这一步依然是最复杂的
//  5. 将构建的继承树存入索引文件
//  6. 同步加载所有的成员声明, 包括函数的声明, 成员方法的声明, 成员变量的声明
//      - IO 密集型
//  7. 同步对各种声明进行检查
//      - CPU 密集型
//      - 依据继承树和IdentifierManager检查声明的类型正确性, 只检查有关类型的部分
//      - 检查函数的重载, 函数签名的比对
//  7. 同步构造expression和Control, 甚至一个文件里不同的expression/control的也可以是同步的
//      - CPU 密集型
//      1. 简单构建, 结构, 不进行类型检查
//      2. 进行类型检查, item的检查, 确认函数签名等
//      3. 进行常量运算-什么是常量, 那些可以看作是常量?
//      4. 转为command sequential
//  8. 一个文件完成之后, 就可以将完全编译后的文件覆盖原来编译的二进制文件
//      - IO密集型

// ParameterizedType 保留成什么样子比较好呢?
// 是保留source, 那可能会导致存太多
// 而且type不一定有能查出结束的能力
//