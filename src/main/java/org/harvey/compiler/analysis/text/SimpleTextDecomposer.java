    ...
    public void appendDecomposed(boolean fullLine, String apartText) {
        ...
        while (amount.sit.hasNext()) {
            ...
            if (SourceFileConstant.SPRITE_SIGN.contains(c)) {
                ...
                add(SourceStringType.MIXED, amount.sb.toString(), endPosition, false);
                amount.sb = add(SourceStringType.SIGN, String.valueOf(c), endPosition, true);
                continue;
            }
            if (c == SourceFileConstant.STRING_ENCIRCLE_SIGN) {
                ...
                amount.sb = add(SourceStringType.MIXED, amount.sb.toString(), beforeString, false);
            ...
            if (Character.isWhitespace(c)) {
                ...
                if (temp == amount.sit) {
                    amount.sb = add(SourceStringType.MIXED, amount.sb.toString(), amount.oldPosition.clone(0, -1),
                            false);
                    continue;
                }
                amount.sit = temp;
                amount.sb = add(SourceStringType.MIXED, amount.sb.toString(), amount.oldPosition, false);
                if (amount.sit == null) {
                    break;
                }
                continue;
