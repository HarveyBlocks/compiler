...
public abstract class SourceFileManager {
     ...
     * 如果返回null, 表示半道崩殂了
     */
    protected StringIterator tryMoveToCharEnd(char c, StringIterator sit, StringBuilder sb,
    ...
    protected StringIterator skipString(int index, StringIterator sit, SourcePosition oldPosition) {
        return skipTextWithPairedSign(index, sit, oldPosition,
                ...
                String.valueOf(SourceFileConstant.STRING_ENCIRCLE_SIGN),
                SourceStringType.STRING, true, SourceFileManager::findTrueStringEnd);
    }
    ...
    protected StringIterator skipTextWithPairedSign(
        ...
        int commitEnd = endIndexFinder.apply(sit.toString(),
                commitPreEnd);// sit.toString().indexOf(post, commitPreEnd);
        if (commitEnd == -1) {
            // 不能在这次做完
    ...
    protected StringBuilder add(SourceStringType nowStatus, String value, SourcePosition sp, boolean start) {
        if (!value.isEmpty()) {
    ...
    protected boolean skipCommitIfExist(char c, DecomposedProcessAmount amount, int index) {
        ...
        if (cnString.equals(SourceFileConstant.SINGLE_LINE_COMMENTS_START)) {
            ...
            amount.sb = add(SourceStringType.MIXED, amount.sb.toString(),
