...
import org.harvey.compiler.execute.test.version1.element.*;
...
public class ArgumentsListHandler implements ExpressionHandler {
    ...
    public boolean handle(ExpressionContext context) {
        ...
        ComplexExpressionWrap[] argumentWarps = injectArguments(
    ...
    private static ComplexExpressionWrap[] injectArguments(
            CallableArgumentOuter callableArgumentOuter,
            List<SourceString> argumentsSource,
            ExpressionContext context,
            PossibleCallableSupplier possibleCallable) {
        if (!argumentsSource.isEmpty()) {
            return new ComplexExpressionWrap[0];
        }
        ...
        while (iterator.hasNext()) {
            ...
            ComplexExpressionWrap argumentWrap = new ArgumentWarp(argument.get(0).getPosition(), argIndex,
        ...
        // 结束了
        ComplexExpressionWrap[] argumentWarps = warps.toArray(new ComplexExpressionWrap[0]);
        // 重新设置possible
