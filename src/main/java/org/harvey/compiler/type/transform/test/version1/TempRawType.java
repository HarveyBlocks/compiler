package org.harvey.compiler.type.transform.test.version1;
...
class AssignableParameterized implements Assignable {
    ...
    public void assign(Parameterized from) {
        ...
        if (from == null) {
            throw new AnalysisDeclareException(SourcePosition.UNKNOWN, "can not 匹配 for from 是 null");
        }
    ...
    private Stack<TempRawType> findPath(TempRawType fromRawType, TempRawType toRawType) {
        ...
        while (cur != toRawType) {
            if (cur.parent == null) {
                throw new AnalysisDeclareException(SourcePosition.UNKNOWN, "没有继承关系");
            }
