...
public class MixedSourceDecomposer {
    ...
    private SourceType nowType;

    public MixedSourceDecomposer(SourceString mixedSource) {
        if (mixedSource.getType() != SourceType.MIXED) {
            throw new CompilerException("Only MIXED source string type is allowed" +
                                        " collectionIn mix string decomposer", new IllegalArgumentException());
        }
        nowType = SourceType.MIXED;
    ...
    private static void analysisDot(
        ...
        if (post == null && pre == null) {
            newPhasedString.add(new SourceString(SourceType.OPERATOR, item.getValue(), item.getPosition()));
            return;
        }
        if (post == null) {
            if (fullNumber(pre.getValue())) {
                newPhasedString.removeLast();
                newPhasedString.add(new SourceString(SourceType.ITEM, pre.getValue() + DOT, pre.getPosition()));
            } else {
                newPhasedString.add(new SourceString(SourceType.OPERATOR, item.getValue(), item.getPosition()));
            }
            return;
    ...
    private static void analysisScientificSign(
        ...
        if (upperScience != -1 && lowerScience != -1) {
            ...
            if (fullNumber(postValue)) {
                newPhasedString.add(new SourceString(SourceType.ITEM, DOT + post.getValue(), item.getPosition()));
                lit.next();
            } else {
                newPhasedString.add(new SourceString(SourceType.OPERATOR, item.getValue(), item.getPosition()));
            }
    ...
    private static boolean checkScienceNotation(
            String postValue, char scienceSign, int scienceIndex,
            SourceString item) {
        if (postValue.indexOf(scienceSign, scienceIndex + 1) != -1) {
            // 如果scienceIndex + 1后还有scienceSign的话
            // item.setType(SourceType.OPERATOR);
    ...
    public SourceTextContext phase() {
        ...
        SourceTextContext.legalTypeAssert(phasedString, Set.of(
                SourceType.STRING, SourceType.SIGN,
                SourceType.MIXED, SourceType.ITEM, SourceType.OPERATOR,
                SourceType.CHAR
        ));
        analysisDot();
        concatItem();
        SourceTextContext.legalTypeAssert(phasedString, Set.of(
                SourceType.STRING, SourceType.SIGN,
                SourceType.ITEM, SourceType.OPERATOR, SourceType.CHAR
        ));
    ...
    private void decomposeSimply() {
        ...
        for (int i = 0; i < source.length(); i++) {
            ...
            if (nowType == SourceType.MIXED) {
                nowType = isOperatorSign(c) ? SourceType.OPERATOR : SourceType.ITEM;
                sb.append(c);
                continue;
            }
            boolean operatorSign = isOperatorSign(c);
            if (nowType == SourceType.OPERATOR && !operatorSign) {
                add();
                this.nowType = SourceType.ITEM;
            } else if (nowType == SourceType.ITEM && operatorSign) {
                add();
                this.nowType = SourceType.OPERATOR;
            }
            sb.append(c);
        }
        add();
    ...
    private void analysisDot() {
        ...
        for (ListIterator<SourceString> lit = phasedString.listIterator(); lit.hasNext(); ) {
            ...
            if (item.getType() != SourceType.MIXED) {
    ...
    private void concatItem() {
        ...
        for (SourceString string : phasedString) {
            SourceType lastType = newPhasedString.isEmpty() ? null : newPhasedString.getLast().getType();
            String value = string.getValue();
            if (lastType == SourceType.ITEM && string.getType() == SourceType.ITEM) {
