...
public class DefaultIdentifierManager implements IdentifierManager {
    ...
    public DefaultIdentifierManager(
        ...
        this.allIdentifierTable = declaredIdentifierTable.stream()
                .map(DefaultIdentifierManager::identifier2Full)
    ...
    public static boolean isFileDeclaration(String identifier) {
        return !identifier.contains(IdentifierPoolFactory.MEMBER);
    }

    private static FullIdentifierString identifier2Full(IdentifierString e) {
        String[] strings = StringUtil.simpleSplit(e.getValue(), IdentifierPoolFactory.MEMBER);
    ...
    private static String joinFullname(String[] fullname) {
        return StringUtil.join(fullname, IdentifierPoolFactory.MEMBER);
    }
    ...
    private void initDeclareMapAndCache() {
        for (int i = 0; i < importReferenceAfterIndex; i++) {
            ...
            StringJoiner joiner = new StringJoiner(IdentifierPoolFactory.MEMBER);
    ...
    public ReferenceElement getReferenceAndAddIfNotExist(FullIdentifierString fullname) {
        ...
        reference = addToImport(fullname);
        if (reference != null) {
            return reference;
        }
    ...
    private ReferenceElement addToImport(FullIdentifierString fullname) {
    ...
    public Integer tryGeneric(String mayGeneric) {
        String genericMark = IdentifierPoolFactory.GENERIC_LIST_PRE_NAME +
                             mayGeneric +
                             IdentifierPoolFactory.GENERIC_LIST_POST_NAME;
    ...
    public IdentifierString getGenericIdentifier(ReferenceElement reference) {
        ...
        if (lastName.startsWith(IdentifierPoolFactory.GENERIC_LIST_PRE_NAME) ||
            lastName.equals(IdentifierPoolFactory.GENERIC_LIST_POST_NAME)) {
            throw new CompilerException(
